...
const Condo = ({data}:CondoProps) => {
  return (
    <div className='flex flex-col bg-white rounded-xl overflow-hidden h-auto w-full  relative '>
        ...
        <h1 className='text-lg font-semibold h-[30px]'>{data.title}</h1>
        <p className='text-md font-semibold mt-5'> {data.location}</p>
        ...
        <div className='flex gap-x-3 items-center text-sm flex-wrap h-[100px] overflow-hidden'>
            <strong>Facilities:</strong>
            {data?.facilities.map(facility => <span key={facility} className='px-3 py-2 bg-gray-50 rounded-xl font-semibold text-zinc-500 cursor-pointer text-sm hover:bg-zinc-300 hover:text-white'>{facility}</span>) }
        </div>
